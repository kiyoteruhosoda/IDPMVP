...
転送した `dist/` の中で `deploy.sh` を実行する。**モードは必須**。冪等（既存 `.env` は上書きしない）。
**ソース不要・ビルドしない**。どのモードでも新イメージを `docker load` し、アプリコンテナ
（api・web・proxy）を `--force-recreate` で必ず作り直す（旧イメージのまま残ったコンテナを確実に置き換える）。

```sh
cd /opt/idp/dist          # 転送先（例）
./deploy.sh migrate       # 初回・DDL 更新（DDL・マスタデータを適用してからアプリを起動）
./deploy.sh app           # 以降のアプリ更新（DDL 変更なし。migrate は実行しない）
```

**初回は DDL 適用が要るため `migrate`（または `reset`）を使う**。以降のアプリのみの更新は `app`。
...
`docker load`（manifest と照合。読込済みならスキップ）→ MariaDB 起動 →〔`migrate` のみ〕マイグレーション
（DDL + マスタデータ）適用 → api・web・proxy を `--force-recreate` で作り直し → `/readyz` で起動確認。
起動できず crash-loop するコンテナ（`restarting`/`exited`）は待たずに検知し、該当モジュールのログを
出して失敗する。DB（mariadb）は落とさない。
...
`migrate` モードは DDL・マスタデータの適用（常駐させない専用 `migrate` サービス）に続けてアプリも
作り直す。DB のみ更新してアプリを触りたくない場合は、`migrate` サービスを直接実行する。

```sh
./deploy.sh migrate       # migrate 適用 + アプリを新イメージで作り直す
# アプリを触らず DDL だけ適用する場合
...
- アプリ: 前のバージョンの `dist/` を残しておき、そこで `./deploy.sh app`（DDL を戻す必要がある場合は
  `migrate`）を実行する（tar から前のイメージが読み込まれる）。
