## 2026-07-23（スマホでテナント一覧の ID が読めない問題を修正）

- **スマホで表内の識別子（`<code>`）を読める形に折り返す**: 一覧テーブルのセル内の識別子（テナント ID・
  `client_id`・`correlation_id` 等）は「折り返さず横スクロールへ逃がす」方針だったが、スマホ（<576px）では
  36 文字の UUID を読むだけで毎回横スクロールが必要になり読みづらかった。ブレークポイントで方針を切り替え、
  タブレット以上（>=576px）は従来どおり 1 行＋横スクロール、スマホ（<576px）は幅を抑えて数行に折り返して
  その場で読めるようにした（`inline-block` + `max-width` で 1 文字ずつ縦積みになるのを防ぐ。`.table-responsive td code`）。
