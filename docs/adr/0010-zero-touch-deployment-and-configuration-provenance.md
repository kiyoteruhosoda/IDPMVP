...
ホスト側の標準入口は `deploy.sh` とし、`app`、`migration`、`reset` のモードを必須にする。
`.env` が無い初回 `app` は、自動生成、DB 起動、migration、アプリ起動、readiness 確認まで実行する。
既存の `init.sh` は互換ラッパーとし、移行期間後に削除できる。

`reset` はデータ破壊操作なので対象を表示し、対話確認または自動化用 `--yes` を要求する。通常の `app` と
`migration` は DB volume や `ENV_LOCKED` を削除しない。
