...
ホスト側の標準入口は `deploy.sh` とし、モードは `app`（DDL 変更なしのアプリ更新）、`migrate`
（DDL・マスタデータ適用＋アプリ更新）、`reset`（DB volume 削除→再構築）の 3 つに限定する。**モード指定は
必須**（引数なしの既定デプロイは廃止し、操作者に app/migrate の選択を強制する）。`.env` が無い初回デプロイは
自動生成し、以後どのモードでも新イメージ load＋アプリコンテナの `--force-recreate`＋readiness 確認まで
実行する（旧イメージのまま残ったコンテナを確実に置き換える）。旧 `init.sh` は削除済み。

`reset` はデータ破壊操作（DB volume 削除）だが、運用上の摩擦を避けるため確認フラグは要求せず
即実行する。`app` と `migrate` は DB volume や `ENV_LOCKED` を削除しない（DB＝mariadb も落とさない）。
