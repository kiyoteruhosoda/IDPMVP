...
### 5. テナント作成フロー（作成者ブートストラップ方式）

テナント作成は親テナント配下への子テナント作成として機能上一律に扱う（§4 の権限制約により実質
root 配下のみ）。作成時に必要な入力は**テナント名のみ**。

| 入力 | 備考 |
|---|---|
| テナント名（`name`） | 表示名。`id`（UUID）はシステムが自動採番する |

**作成者自身が新テナントのブートストラップ管理者になる。** 初期管理者メール・自動生成パスワードは
受け取らず、平文パスワードの受け渡しも発生しない。作成トランザクション（unit of work）では次を
原子的に永続化する。

1. 新テナント行。
2. **作成者（`actor`）の ACTIVE な GUEST メンバーシップ**（所属元は親テナントのまま。招待フローは
   経ない）。
3. 作成者へ新テナント scope の `idp.tenant.admin` 付与。

作成後の運用フロー（作成者が自身の SSO セッションのまま新テナントの管理コンソールで行う）:

1. 正式な管理者（新テナント所属の HOME 利用者）をメンバー画面から作成する。
2. その利用者へ `idp.tenant.admin` を付与する。
3. 作成者は自身のゲストメンバーシップを解除して離脱する（解除時に当該テナント scope の権限行も
   後始末される。§3）。

この結果、最終状態は「新テナント所属の HOME 管理者が居り、root の system.admin は当該テナント内部を
操作できない」となる。**ブートストラップ中に限り**作成者（root の system.admin）は新テナント内部を
操作できる（従来の「作成者は一切内部を操作できない」から緩和。トレードオフとして、離脱を運用者の
責務とする）。

- 作成 API のレスポンスは作成したテナントそのもの（`generated_password` は返さない）。
- 正式な管理者ユーザーはメンバー画面からの利用者作成で生成され、自動生成パスワード＋
  `must_change_password = TRUE` を持つ（初回ログイン時にパスワード変更画面へ誘導。以下は不変）。
