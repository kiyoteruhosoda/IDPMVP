...
DDD 4層を Rust モジュールへ写像した cargo workspace。ADR-0007（API/Web サービス分割）の P1 で
単一クレートを `core`（DB 直結：domain/application/infrastructure＋設定・ログ基盤）と
`api`（axum presentation＋バイナリ）へ分割した。web / contracts crate は後続フェーズ（P3）で追加する。
...
i18n/                 # fluent 翻訳（.ftl）。api から ../../i18n で埋め込み（P3 で web crate へ移設予定）
```
