...
--   * 既定照合は utf8mb4_bin（完全一致・大文字小文字を区別）。CITEXT 相当（email /
--     preferred_username）のみ大小無視の utf8mb4_unicode_ci を列単位で指定する。
--   * ハッシュ・kid 等の ASCII 固定値は ascii/ascii_bin。
...
CREATE TABLE auth_sessions (
    -- id は 128bit 以上の推測不能なランダム値（256bit の base64url トークンを想定）。
...
CREATE TABLE signing_keys (
    kid                    VARCHAR(128) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
