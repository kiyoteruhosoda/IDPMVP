...
admin-nav-user-search = Find user
admin-nav-user-search-desc = Search users by email or username and open their permissions
...
admin-settings-runtime-restart-required = Restart required to apply
admin-settings-runtime-secret = secret (hidden)
admin-settings-runtime-source-default = Default
admin-settings-runtime-env-locked-note = An environment variable is set and takes precedence over the DB value.
...
admin-settings-runtime-edit-note = Saving with an empty value removes the DB override (falls back to the environment variable or default). Changes take effect after the API service restarts.
...
admin-saml-client-import-file-hint = Upload the service provider's SAML metadata XML file (.xml) to auto-fill the fields below. Takes precedence over the pasted XML.
