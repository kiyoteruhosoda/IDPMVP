...
admin-nav-tenants = テナント登録
admin-tenants-title = テナント登録
...
admin-tenants-self-registration = 自己登録
...
admin-home-group-integration-desc = RP クライアントと署名鍵を管理します。
...
admin-nav-tenants-desc = 新しいテナントを登録
...
admin-settings-runtime-note = 値は表示せず、設定キーごとの管理区分・現在の出所・状態・理由・再起動要否を表示します。secret の値や fingerprint は表示しません。
