...
pass "/authorize → /login 302・auth_session Cookie 発行"

csrf="$(curl -fsS -b "$CJAR" "${WEB}/login" | grep -oE '[a-f0-9]{64}' | head -1)"
[[ -n "$csrf" ]] || fail "web /login がフォーム（CSRF）を返しません"
...
AJAR="$(mktemp)"
acsrf="$(curl -fsS -c "$AJAR" "${WEB}/admin/console/login" | grep -oE '[a-f0-9]{64}' | head -1)"
curl -fsS -b "$AJAR" -c "$AJAR" -o /dev/null -X POST "${WEB}/admin/console/login" \
  -H 'content-type: application/x-www-form-urlencoded' \
  --data-urlencode "username=admin" --data-urlencode "password=ChangeMe!123" --data-urlencode "csrf_token=${acsrf}"
curl -fsS -b "$AJAR" "${WEB}/admin/console" | grep -q "/admin/console/clients" || fail "管理ホームが描画されません（whoami 経由）"
pass "admin ログイン → ホーム描画（web→api /admin/whoami）"

ccsrf="$(curl -fsS -b "$AJAR" "${WEB}/admin/console/clients/new" | grep -oE 'name="csrf_token" value="[a-f0-9]{64}"' | grep -oE '[a-f0-9]{64}')"
created="$(curl -fsS -b "$AJAR" -X POST "${WEB}/admin/console/clients/new" \
  -H 'content-type: application/x-www-form-urlencoded' \
...
curl -fsS -b "$AJAR" -o /dev/null -w '%{http_code}' "${WEB}/admin/console/status"     | grep -q 200 || fail "状況画面が 200 を返しません"
curl -fsS -b "$AJAR" -o /dev/null -w '%{http_code}' "${WEB}/admin/console/audit-logs"  | grep -q 200 || fail "監査画面が 200 を返しません"
pass "状況・監査画面（web→api /admin/clients/status・/admin/audit-logs）"

# 利用者検索→権限付与→剥奪。
sr="$(curl -fsS -b "$AJAR" "${WEB}/admin/console/users?q=${U}@example.com")"
